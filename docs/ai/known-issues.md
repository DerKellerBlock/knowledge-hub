...
- **LIM-007:** `jinaai/jina-reranker-v2-base-multilingual` (via `KH_RERANKER_MODEL`-Env-Var) ist implementiert aber ungetestet. Der jina-Modell-Download (~1.1 GB), `trust_remote_code=True`-Ladevorgang, `einops`-Import, sigmoid-Score-Skala (0–1) und Re-Evaluation wurden nie mit echtem Modell ausgeführt (Default bleibt `cross-encoder/ms-marco-MiniLM-L-12-v2`). Vor einem dauerhaften Switch auf jina: Download freigeben, `KH_RERANKER_MODEL=jinaai/jina-reranker-v2-base-multilingual python scripts/quality/run_evaluation.py --domain godot` ausführen, Regression prüfen. jina-Reranker-Test ist NACH Phase 2a geplant.
- **LIM-008:** BGE-M3 multilingual embeddings + ms-marco English-only reranker = Übergangskonfiguration (Phase 2a). BGE-M3 (`KH_EMBEDDING_MODEL=BAAI/bge-m3`) erzeugt multilingual starke Candidates, aber der Stage-2-Reranker (`cross-encoder/ms-marco-MiniLM-L-12-v2`) ist English-only und kann gute deutsche BGE-M3-Candidates verwerfen. jina-reranker-v2-multilingual-Test (LIM-007) ist die geplante Lösung, wird aber erst nach Phase 2a ausgewertet.
