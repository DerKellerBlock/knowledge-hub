# Known Issues — Knowledge Hub

## Behobene Probleme

- **KI-001:** Phase 1+2 abgeschlossen (2026-06-09) — Alle Kern-Komponenten implementiert
- **KI-002:** Godot-Skills ins Hub migriert — Skills sind Single Source of Truth
- **TD-001:** ChromaDB-Integration getestet — 265 MB Index, 18.222 Chunks, Cosine-Metrik
- **KI-003:** Retrieval 2.0 implementiert (2026-06-10) — BM25 ersetzt ripgrep, Cross-Encoder-Reranking, Plugin-basiertes strukturiertes Parsing
- **KI-004:** godot-007 (3D character controller) Retrieval-Lücke geschlossen (2026-06-30) — `tips.md`/CharacterBody3D Stair Stepping-Sektion um GDScript-Code-Snippet erweitert (Godot-4-Stable-APIs + PR-#114447-APIs klar gekennzeichnet). Composite 0.7136 → 0.8594, SR 0.6667 → 1.0, `tips.md` Top-2 statt Rank 32. Cross-Encoder-Score -8.53 → +0.71. Behebt BM25-Token-Overlap (Query-Keywords velocity/gravity/jump/move_and_slide jetzt als Tokens vorhanden) und Cross-Encoder-Kontext (Sektion liest sich jetzt als vollständiger Character-Controller).
- **KI-005:** LIM-005 resolved (2026-06-30) — Alle 29 `solution_summary`-Felder in `real_world_sources` kuratiert (15 godot + 14 davinci_resolve, Commit 5a07b4b). Solution Alignment (Ebene 2) der Real-World-Evaluation ist jetzt vollständig evaluierbar.
- **KI-006:** LIM-007 resolved (2026-07-01) — jina-reranker-v2-base-multilingual tested with BGE-M3 embeddings. godot avg_composite 0.8594 (identisch), davinci 0.7304 (+0.0058), keine Regression. godot-008 multilingual gain: deutsche faq.md auf Rang 1 trotz englischer Query. Adopted via `KH_RERANKER_MODEL` env var; CI quality-gate default seit 2026-07-01. CC-BY-NC-4.0 akzeptiert für persönlichen nicht-kommerziellen Hub.
- **KI-007:** LIM-008 resolved (2026-07-01) — BGE-M3 multilingual embeddings + jina multilingual reranker = konsistente Multilingual-Pipeline. CI quality-gate nutzt jina default. Lokal via `KH_RERANKER_MODEL` env var aktivierbar (Shell-Profile/`.env` setup siehe `docs/ai/best-practices.md`). Ohne Env-Var fällt System auf ms-marco zurück (Übergangskonfiguration bleibt funktionsfähig, ist aber nicht mehr der empfohlene Pfad).

## Technische Schulden

- **TD-002 (resolved 2026-06-29):** Quality Evaluation Platform Phase 1+2 implementiert — Golden Dataset für `godot` und `davinci_resolve`, Bewertungsrubrik, CLI-Skripte (`run_evaluation.py`, `add_question.py`, `validate_dataset.py`, `generate_report.py`), E2E Quality Tests. Siehe `docs/superpowers/specs/2026-06-29-knowledge-hub-quality-evaluation-platform-design.md`.

## Einschränkungen

- **LIM-001:** MCP-Server nur stdio (kein HTTP/SSE) — akzeptabel für persönlichen Hub
- **LIM-002:** `section_path` und `chunk_type` fehlen bei DaVinci-Resolve-Fallback-Chunks (Repo-Quellen) und bei Preamble-Chunks aus `markdown_section_chunk()`. Sektions-Chunks aus `markdown_section_chunk()` haben jetzt `chunk_type="personal_section"` und `name=<Sektionsüberschrift>`, aber `section_path=None`. Godot-Repo-Chunks haben diese Felder via rst-godot-Parser. Agenten dürfen sich nicht auf `section_path` als zuverlässiges Feld verlassen.
- **LIM-003:** `text`-Feld in Suchergebnissen wird auf 5000 Zeichen trunciert (`hybrid_search.py:127`, `embed_search.py:69`). DaVinci-Fallback-Chunks können bis ~8000 Zeichen groß sein. Der Orchestrator-Prompt instruiert Agenten, Truncation zu erkennen und nicht zu halluzinieren.
- **LIM-004:** `expected_page_ranges` in `quality/golden/davinci_resolve.yaml` enthalten ±2 Seitentoleranz (Chunking-Variance), nicht die exakten Seitenzahlen. Die PMA-Bewertung (`score_page_metadata_accuracy`) prüft, ob die tatsächlichen `page_start`/`page_end`-Werte innerhalb der erwarteten Range ±2 liegen. **Klarstellung (2026-07-07, Task D2):** `page_start`/`page_end` in `late_chunk`-Chunks sind **0-basiert** (PyMuPDF4LLM-Konvention, analog `image_match.page` per VRF-001). Verifiziert via `pdftotext` Cross-Check für „The Cut page Timeline controls" (Treffer `page_start=521` → Text auf PDF-Seite 522). Die ±2 Toleranz bezieht sich auf Chunking-Varianz, nicht auf die Basis. Siehe `docs/issues/vqa-opencode-passthrough/context/page-offset-verification.md`.
- **LIM-005 (resolved 2026-06-30):** `solution_summary` in `real_world_sources` war `null` für alle 14 Golden-Dataset-Fragen (TODO-Platzhalter). Kuratiert in Commit 5a07b4b: 15 godot-Summaries + 14 davinci_resolve-Summaries (29 total, alle ausgefüllt). Solution Alignment (Ebene 2) der Real-World-Evaluation ist jetzt vollständig evaluierbar.
- **LIM-006:** `line_end` in `markdown_section_chunk()` und `fallback_chunk()` kann 1 Zeile über den tatsächlichen Inhalt hinausgehen (konsistente Konvention, aber semantisch ungenau). Risiko niedrig, da Orchestrator den `text`-Inhalt zeigt, nicht Zeilennummern.
- **LIM-007 (resolved 2026-07-01):** `jinaai/jina-reranker-v2-base-multilingual` getestet mit BGE-M3-Embeddings. godot avg_composite 0.8594 (identisch), davinci 0.7304 (+0.0058), keine Regression. godot-008 multilingual gain (deutsche faq.md auf Rang 1). Adopted via `KH_RERANKER_MODEL` env var; CI quality-gate default. Siehe KI-006.
- **LIM-008 (resolved 2026-07-01):** BGE-M3 multilingual embeddings + jina multilingual reranker = konsistente Multilingual-Pipeline. CI quality-gate nutzt jina default. Lokal via `KH_RERANKER_MODEL` env var aktivierbar. Ohne Env-Var fällt System auf ms-marco zurück. Siehe KI-007.
- **LIM-009 (B10, teilweise adressiert durch Phase 2.2 Late Chunking):** BGE-M3 long-context verändert das effektive Chunking-Verhalten. BGE-M3 `max_seq_length=8192` vs all-mpnet 384. DaVinci-Fallback-Chunks bis ~8000 Zeichen (~2000 Token) werden jetzt VOLLSTÄNDIG verarbeitet (vorher auf 384 Token trunciert). Das war ein Konfounder in der Phase-2a-Re-Evaluation: Embedding-Modell und effektive Chunk-Länge änderten sich gleichzeitig. Phase 2.2 (Late Chunking) ist ein weiterer Chunking-Wechsel, der den Konfounder NICHT auflöst — die DaVinci-Re-Evaluation misst jetzt BGE-M3-Long-Context + Late Chunking + 12.367 Chunks gleichzeitig. Eine isolierte Messung des Chunking-Effekts wäre nur mit einem 384-Token-BGE-M3-Setup möglich, das nicht geplant ist.
- **LIM-010:** `line_start`/`line_end = 0` für Late-Chunk-Chunks (Phase 2.2). PDF-Chapter haben keine Zeilennummern, da die Chapter-Grenzen aus den extrahierten Markdown-Headern kommen, nicht aus Zeilen-Positionen. `markdown_section_chunk()` und `fallback_chunk()` produzieren weiterhin sinnvolle `line_start`/`line_end` für textbasierte Quellen. Konsumenten, die Zeilennummern erwarten, müssen `chunk_type=="late_chunk"` als Ausnahme behandeln. (LOW-2 aus Diff-Review.)
- **LIM-011 (resolved 2026-07-04):** BGE-M3 MPS Hang resolved. `torch` 2.12.0 behebt den BGE-M3 + `transformers` 4.57.6 MPS-Deadlock, der `device='cpu'` erzwungen hat. `model_manager.get_embedder()` nutzt jetzt die `KH_EMBEDDING_DEVICE` env var (default `cpu`, opt-in `mps`, ~4.7× Speedup auf Apple Silicon). Cache-Key ist `embedder:<model>:<device>` (Runtime-Switch lädt frische Instanz). Pre-Flight-Mitigation (R1.1): 100-Chunk MPS-Encode vor jedem großen Build; bei Hang (>30 s) auf CPU zurückfallen. Integration-Test `test_mps_encode_pre_flight` automatisiert den Check. Siehe Phase 3.3a Spec `docs/superpowers/specs/2026-07-04-acceleration-mps-parallel-design.md`.
- **LIM-012:** Ollama-Version für Gemma 4 erforderlich + Gemma 4 12B ist ein Reasoning-Modell (Phase 3.1a Erkenntnis). (1) Gemma 4 12B MLX erfordert Ollama v0.31.1+ (MTP-Speedup); ältere Versionen (z.B. v0.24.0, von der Ollama.app installiert) lehnen den Pull mit HTTP 412 ab ("requires a newer version of Ollama"). Fix: Homebrew-Ollama auf v0.31.1 upgraden, alte Ollama.app entfernen (Pfad-Konflikt via totem Symlink `/usr/local/bin/ollama` — kosmetisch, PATH löst korrekt auf `/opt/homebrew/bin/ollama` da früher im PATH). (2) Gemma 4 12B generiert vor der finalen Antwort eine interne Thinking-Phase (`message.thinking`-Feld in der Ollama-Antwort). Mit dem Anthropic-Default `num_predict=100` bleibt `content=''` (`done_reason='length'`). `generate_context()` nutzt daher `num_predict=800` als Default (~256 eval-Tokens: 949 Zeichen Thinking + ~15 Token Antwort). Bei ~16.6s pro Chunk skaliert das auf ~15.000 Godot-Chunks = ~69 Stunden (reiner LLM-Teil) — länger als die 15–20h Spec-Schätzung. Die Durchsatz-Planung in Phase 3.1b/c muss das berücksichtigen.

- **LIM-013:** DaVinci-Fallback document_text-Truncation auf 50K Zeichen (NB-2). Bei PDFs, die mehrere MB Markdown sind, ist der Kontext auf die ersten 50K Zeichen limitiert — möglicherweise nutzlos, wenn der Chunk aus Kapitel 47 stammt. Akzeptiert: Fallback-Chunks sind selten (nur bei late_chunk-Fehler). `_truncate()` in `model_manager.py` begrenzt `document_text` auf `_MAX_DOCUMENT_CHARS = 50_000` Zeichen und `chunk_text` auf `_MAX_CHUNK_CHARS = 30_000` Zeichen (Phase 3.1b M3-Mitigation).
- **Phase 3.1c Ergebnis:** Contextual Retrieval (gemma4:cloud) zeigt einen realen, aber kleinen Benefit auf dem Godot-Domain: +0.0105 avg_composite, 1 von 21 Fragen (godot-012) von weak → pass gehoben, keine Regressionen.composite_delta liegt unter der +0.02 Schwelle → kein produktiver Rollout. godot-008 (Sprachbarriere) und godot-009 (breite Animation) sind durch Contextual Retrieval allein nicht lösbar. Eval-Domains (godot_eval_a/b) behalten für spätere Re-Läufe (Contextual BM25, Prompt-Tuning, anderes Modell).
- **Phase 3.2 Ergebnis:** Contextual BM25 (C) erreicht GO: +0.0209 avg_composite (A=0.8281, C=0.8490), 20 pass / 1 weak. godot-008 (Sprachbarriere) gehoben durch Contextual BM25. Nur noch godot-009 (AnimationTree, breite Animation) weak. R4: Eval nutzt fallback_chunk, produktiv nutzt rst-godot → Promote braucht separaten Cloud-Lauf.
- **Phase 3.3b Ergebnis (DaVinci Smoke-Test, 2026-07-06):** Infrastruktur-Validierung erfolgreich. eval_a (Baseline) vs eval_b (Contextual+BM25) beide avg_composite 0.8035, 20 pass / 0 weak, PMA 0.755. Delta = 0.0 (nur 6 kontextualisierte Chunks → statistisch nicht aussagekräftig, wie erwartet). R4 DaVinci Cache-Hit bestätigt (6/6 hits). Promote durchgeführt: produktive `davinci_resolve` hat jetzt Contextual Retrieval + Contextual BM25 (6 ui-map.md Path-A Chunks, 12.555 late_chunk unverändert). Keine Regression. Eval-Domains entfernt.
- **Phase 3.3c Ergebnis (Godot-Promote, 2026-07-06):** Produktion hat jetzt Contextual Retrieval + Contextual BM25 (24.593 Chunks, alle context_prefix != None). **Leichte Regression vs Pre-Promote-Baseline:** avg_composite 0.8386 → 0.8281 (−0.0105), 19 pass / 2 weak → 18 pass / 3 weak, SR 0.9524 → 0.9286. **godot-017 (Performance/LOD) fiel pass → weak** (SR 1.0 → 0.5) — Contextual BM25 hat die Performance-bezogenen Chunks in best-practices.md schlechter auffindbar gemacht (vermutlich weil der context_prefix die BM25-Token-Verteilung verschoben hat). godot-008 bleibt pass (Sprachbarriere nicht verschlechtert), godot-009 + godot-012 bleiben weak. Die Phase 3.2 eval_c-Erwartung (0.8490) wurde nicht erreicht — R4 dokumentiert: produktiv rst-godot ≠ eval fallback_chunk → andere Chunk-Struktur, Contextual Retrieval wirkt anders. Decision: Produktion behält Contextual Retrieval (Infrastructure works, godot-008 pass, Regression klein). godot-017 Regression unten dokumentiert. Backup `chromadb_data/godot.bak-pre-3-2/` behalten für Rollback.

- **Spot-Check-Gate-Limitation (Phase 3.1b):** Der Spot-Check misst NICHT echte Quality gegen `godot.yaml` (nur 2 pure-personal-Fragen, N=2 ist schwaches Signal). Das Gate ist ein reines No-Go-Gate (composite-Delta < −0,02 → Abbruch). Echte Quality-Entscheidung folgt in 3.1c (Voll-Lauf gegen `godot.yaml`). 3.1c-Go ist separate Noah-Entscheidung.

## Bekannte Retrieval-Lücken

- **godot-008 (3D model visibility, weak):** Composite 0.6406, SR 0.5 (`godot-docs-reference-packed.md` gefunden, `faq.md` fehlt Top-10). Wurzelursache ist die Sprachbarriere: die englische Query („Why is my 3D model not visible") matcht semantisch schlecht gegen die deutsche `faq.md`-Sektion, da `all-mpnet-base-v2` English-only ist. Phase-1-Content-Maßnahmen (neue FAQ-Sektion) können das nicht beheben. Eine deutsche Variante `godot-008-de` testet `faq.md` direkt ohne Sprachbarriere. Die echte Lösung ist (b) BGE-M3 multilingual in Phase 2. Weitere Optionen: (a) FAQ-Visibility-Abschnitt mit englischen Suchankern verstärken, (c) Frage spezifischer formulieren.

- **godot-009/012 (Phase 2.4 weak, composite 0.6406):** Zwei weak-Fragen (Animation, Navigation) aus der Phase-2.4-Erweiterung. Content-Maßnahmen in `tips.md` (AnimationTree+BlendSpace2D Locomotion, NavigationAgent3D Enemy Chase) haben nicht ausgereicht, um die Fragen auf pass zu heben. BM25-Token-Overlap bleibt niedrig, weil die Themen breit gestreut sind und die API-Referenz-Chunks fragmentiert bleiben. Diese Fragen bleiben als bekannte Retrieval-Lücken dokumentiert.
- **godot-017 (Performance/LOD, weak nach Phase 3.3c Promote):** Composite 0.6406, SR 0.5. War pass (0.8594, SR 1.0) in Pre-Promote-Baseline, fiel durch Contextual BM25 auf weak. Contextual BM25 hat die Performance-bezogenen Chunks in `best-practices.md` (3D Performance LOD/Occlusion/Visibility) schlechter auffindbar gemacht — vermutlich weil der context_prefix die BM25-Token-Verteilung verschoben hat. Rollback-Möglichkeit: `chromadb_data/godot.bak-pre-3-2/`.

- **DaVinci Phase-1-Baseline-Verlust:** DaVinci avg_composite 0.7218 / PMA 0.1714 ist die erste Messung nach Phase 1 (Chunk-Overlap 200→400, +283 Chunks). Keine Pre-Phase-1-Baseline vorhanden (Backups entfernt). PMA 0.1714 ist niedrig (±2 Seitentoleranz, PDF-Chunking-Varianz) — ob Phase 1 die PMA verschlechtert oder verbessert hat, ist nicht ermittelbar. Phase 2 sollte eine neue DaVinci-Baseline etablieren und den Chunk-Overlap-Effekt auf PMA isoliert messen.


## Vision Retrieval Feature (2026-07-07)

- **VRF-001:** Bild-Extraktion verwendet PyMuPDF4LLM `write_images=True`,
  das alle Bilder als PNG extrahiert. PyMuPDF4LLM benennt Bilder
  `<pdf_stem>-<page>-<idx>.png` mit 0-basierten Seitenzahlen. Das
  `page`-Feld im Manifest ist 0-basiert (abweichend von `page_start`/
  `page_end` in Text-Chunks, die 1-basiert sind). Konsumenten müssen
  den Unterschied beachten.
- **VRF-002:** Context-Aware Captions können trunzierte Bild-Referenzen
  im Kontext enthalten wenn das ±200-char-Fenster mitten in einer
  `![](path.png)`-Ref schneidet. `extract_pdf_images.py` hat eine Regex
  die trunzierte Refs stript, aber Edge-Cases sind möglich. Captions
  sind daher gelegentlich leicht verrauscht — für RAG akzeptabel.
- **VRF-003:** 4-Listen-RRF Merge-Strategie reserviert 1/3 der Top-K-
  Slots für Bilder (min 1 wenn Bilder im RRF-Pool). Das kann Text-
  Treffer mit höherem Cross-Encoder-Score verdrängen. Für text-zentrierte
  Queries ist das suboptimal; für bild-zentrierte Queries ist es
  erforderlich. Die 1/3-Schwelle ist ein Kompromiss (Adjustierbar im
  Code: `top_k // 3`).
- **VRF-004:** Bild-Embeddings (SigLIP-2, 1152 dims) und Text-Embeddings
  (BGE-M3, 1024 dims) leben in unterschiedlichen Vektorräumen. Die
  4-Listen-RRF kombiniert sie über RRF-Rang (dim-unabhängig), nicht über
  Cosine-Similarity. Eine direkte Cross-Modality-Similarity ist nicht
  möglich (Modality-Gap, siehe Spheron Benchmark).
- **VRF-005:** `image_bm25_search` liefert Bild-Treffer mit `image_id`
  als `chunk_id`. `_resolve_image_metadata` reichert sie mit Caption-
  Metadaten aus der `<domain>_images::cap`-Collection an. Wenn ein
  Bild keine Caption hat (z.B. `quality=poor` übersprungen), fehlen
  `caption`, `image_path` etc. — der Treffer wird stillschweigend
  verworfen.

## Visual Question Answering (2026-07-07)

- **VQA-001:** `image_path` funktioniert nur für Domains mit indexierten
  Screenshots. Aktuell hat nur `davinci_resolve` eine `<domain>_images`
  Collection. Andere Domains (godot, …) liefern keine `image_match`
  Treffer — `image_similarity_search` returniert `[]` (graceful).
- **VQA-002:** Keine OCR-Texterkennung. SigLIP-2 findet *ähnliche*
  Screenshots per visueller Ähnlichkeit, extrahiert aber keinen Text
  aus dem Nutzer-Bild. Wenn das Nutzer-Bild Text enthält, der nicht in
  einem DaVinci-Screenshot vorkommt, kann die Suche schwach sein.
- **VQA-003:** Caption-basierte Antwortqualität. Die `image_match`
  Treffer liefern die Caption des ähnlichsten indexierten Screenshots,
  nicht eine direkte Beschreibung des Nutzer-Bildes. Bei sehr ähnlichen
  Screenshots (sim > 0.9) ist die Caption eine gute Beschreibung; bei
  niedrigerer Similarity kann die Caption irreführend sein.
- **VQA-004:** MiniMax M3 Vision-LLM Integration ist **deferred**. Die
  Caption-basierte Antwort funktioniert ohne Vision-LLM, würde aber von
  einem direkten Bild-Vergleich (Nutzer-Bild + Top-3 Screenshots an
  Vision-LLM) profitieren. Siehe
  `docs/issues/visual-question-answering/spec.md` Section 5.
- **VQA-005:** `image_match` Treffer werden PREPENDED (additiv, bis
  top_k). Die kombinierte Liste kann bis zu 2*top_k Einträge enthalten
  (top_k image_match + top_k text/image/caption). Consumers die ein
  strikt top_k-limit brauchen, müssen selbst abschneiden.
