...
3. _basis.x, -cam_basis.get_euler().x) 	dir = cam_basis * dir  	# Limit the input to a length of 1. `length_squared()` is faster to check 	# than `length()`. 	if dir.length_squared() > 1: 		dir /= dir.le
