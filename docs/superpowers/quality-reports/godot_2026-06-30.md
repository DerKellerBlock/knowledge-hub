...
3. ## Area3D Gravity Override + CharacterBody3D - **Problem:** `CharacterBody3D.get_gravity()` ignoriert Area3D-Gravity-Overrides (RigidBody3D respektiert sie) - **Ursache:** Bug in `get_gravity()` — Are
