...
- **Composite Score:** 0.8351
- **Pass:** 8 (88.9%) | **Weak:** 1 (11.1%) | **Fail:** 0 (0.0%)

## Metric Averages
| Metric | Average |
|--------|---------|
| Source Recall | 0.9444 |
...
- **Found sources:** ['godot-docs-reference-packed.md', 'godot-docs-3d-packed.md']
- **Recommendation:** Review index coverage for this question.

## Truncation Warnings
- godot-004: 5 result(s) with text >= 5000 chars (heuristic, see LIM-003).
- godot-006: 2 result(s) with text >= 5000 chars (heuristic, see LIM-003).
- godot-007: 3 result(s) with text >= 5000 chars (heuristic, see LIM-003).
- godot-008: 5 result(s) with text >= 5000 chars (heuristic, see LIM-003).
- godot-008-de: 3 result(s) with text >= 5000 chars (heuristic, see LIM-003).
...
2. using Grow.  Note that in Godot 4.5 onwards, stencil buffer-based outlines are available using the **Outline** :ref:`stencil mode <doc_standard_material_3d_stencil>`. This can be used as an alternativ
3. Class: CameraFeed extends RefCounted → Object  :github_url: hide CameraFeed ========== A camera feed gives you access to a single physical camera attached to your device. Description ----------- A cam
...
1. ## CharacterBody3D Stair Stepping - CharacterBody3D hat native stair stepping (PR #114447, 2025-12-30, open): Properties `step_enabled`, `step_height` (default 0.3m), `step_smooth_enabled`, `step_smoo
2. hen reassigned. 		velocity.x = move_toward(velocity.x, 0, STOP_FORCE * delta) 	else: 		velocity.x += walk * delta 	# Clamp to the maximum horizontal movement speed. 	velocity.x = clamp(velocity.x, -WA
3. Method: CharacterBody3D.get_last_motion Inherits: PhysicsBody3D → CollisionObject3D → Node3D → Node → Object  Vector3 get_last_motion ( ) Returns the last motion applied to the CharacterBody3D during
...
2. Property: GLTFNode.visible Signature: bool visible = true Inherits: Resource → RefCounted → Object  bool visible = true - set_visible ( value\: bool ) - bool get_visible ( ) If true, the GLTF node is
3. ad of   **Pixel Alpha**. This will make the material opaque, which also speeds up rendering.  .. figure:: img/3d_rendering_limitations_transparency_sorting.webp    :align: center    :alt: Transparency
...
3. ## 3D  ### ChurchData.scale Drei-Modus-Semantik - **Pattern:** Drei Modi je nach GLB-Typ: (a) native bottom-origin, (b) v2-Platzhalter-Kompensation, (c) real-world units - **Warum:** Meshy-Modelle kom
